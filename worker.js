...
const renderSocialLinks=value=>{const e=document.getElementById("footerLinks");if(!e)return;const admin=e.querySelector('a[href="/admin"]');String(value||"").split(/\\n+/).map(x=>x.trim()).filter(Boolean).forEach(line=>{const parts=line.split("|"),label=(parts.shift()||"").trim(),url=parts.join("|").trim(),href=safeUrl(url);if(!label||!href)return;const a=document.createElement("a");a.className="admin-link";a.href=href;a.target="_blank";a.rel="noopener";a.textContent=label;e.insertBefore(a,admin)});};
