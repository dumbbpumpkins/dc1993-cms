...
const renderSocialLinks=value=>{const footer=document.getElementById("footerLinks"),header=document.getElementById("headerSocials");if(footer)footer.querySelectorAll(".dynamic-social").forEach(x=>x.remove());if(header)header.innerHTML="";const items=String(value||"").split(/\\n+/).map(x=>x.trim()).filter(Boolean).map(line=>{const parts=line.split("|"),label=(parts.shift()||"").trim(),url=parts.join("|").trim(),href=safeUrl(url);return label&&href?{label,href}:null}).filter(Boolean);items.forEach(item=>{if(footer){const admin=footer.querySelector('a[href="/admin"]'),a=document.createElement("a");a.className="admin-link dynamic-social";a.href=item.href;a.target="_blank";a.rel="noopener";a.textContent=item.label;footer.insertBefore(a,admin)}if(header){const a=document.createElement("a");a.href=item.href;a.target="_blank";a.rel="noopener";a.textContent=item.label;a.onclick=()=>document.getElementById("nav").classList.remove("open");header.appendChild(a)}});if(header)header.style.display=items.length?"flex":"none";};
...
<div class="grid"><div class="full"><label>Small heading</label><input id="updates_eyebrow" type="text"></div><div class="full"><label>Heading</label><input id="updates_heading" type="text"></div><div class="full"><label>Text</label><textarea id="updates_text"></textarea></div></div>
