...
const safeInstagramUrl=u=>{const raw=String(u??"").trim();if(!raw)return"";try{const x=new URL(raw);const host=x.hostname.toLowerCase().replace(/^www\./,"");if(host!=="instagram.com")return"";if(!/^\/(p|reel|tv)\//.test(x.pathname))return"";return x.href}catch{return""}};
